...
QtObject {
    ...
    function pageDisplayWidth() { return pageSourceWidth * pageScale() }
    function pageDisplayHeight() { return pageSourceHeight * pageScale() }
    function pageLeft() { return (canvasWidth - pageDisplayWidth()) / 2 }
    function pageTop() { return (canvasHeight - pageDisplayHeight()) / 2 }
    function viewDocScale() { return pageScale() * zoom }
    function livePreviewScale() { return Math.min(1.0, viewDocScale()) }
    function documentToViewLength(value) { return value * viewDocScale() }
    function documentToViewX(x) { return panX + (pageLeft() + x * pageScale()) * zoom }
    function documentToViewY(y) { return panY + (pageTop() + y * pageScale()) * zoom }
    function viewToDocumentX(x) { return (x - panX) / viewDocScale() - pageLeft() / pageScale() }
    function viewToDocumentY(y) { return (y - panY) / viewDocScale() - pageTop() / pageScale() }

    function zoomStateAt(x, y, factor) {
        const oldZoom = zoom
        const nextZoom = Math.max(minimumZoom, Math.min(maximumZoom, oldZoom * factor))
        if (nextZoom === oldZoom)
            return { zoom: oldZoom, panX: panX, panY: panY, changed: false }
        return {
            zoom: nextZoom,
