...
QtObject {
    ...
    function commitHandles() {
        ...
        if (activePerspectiveHandle === "n") return [
            { name: "nw", x: perspectiveStartNwX, y: perspectiveStartNwY + dy },
            { name: "ne", x: perspectiveStartNeX, y: perspectiveStartNeY + dy }
        ]
        if (activePerspectiveHandle === "e") return [
            { name: "ne", x: perspectiveStartNeX + dx, y: perspectiveStartNeY },
            { name: "se", x: perspectiveStartSeX + dx, y: perspectiveStartSeY }
        ]
        if (activePerspectiveHandle === "s") return [
            { name: "sw", x: perspectiveStartSwX, y: perspectiveStartSwY + dy },
            { name: "se", x: perspectiveStartSeX, y: perspectiveStartSeY + dy }
        ]
        if (activePerspectiveHandle === "w") return [
            { name: "nw", x: perspectiveStartNwX + dx, y: perspectiveStartNwY },
            { name: "sw", x: perspectiveStartSwX + dx, y: perspectiveStartSwY }
        ]
        return [{ name: activePerspectiveHandle, x: perspectiveX, y: perspectiveY }]
    }
