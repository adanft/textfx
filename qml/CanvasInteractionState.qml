...
QtObject {
    ...
    function updatePointer(x, y, pressed) {
        ...
        if (!pressed)
            return { panned: false, panDx: 0, panDy: 0, creating: false }
        ...
        return { panned: panned, panDx: panned ? dx : 0, panDy: panned ? dy : 0, creating: creating }
    }
    ...
    function endRelease() {
        const rectangle = dragMode === createMode ? createRectangle() : { x: 0, y: 0, width: 0, height: 0 }
        const wasCreating = dragMode === createMode
        dragMode = idleMode
        return { creating: wasCreating, rectangle: rectangle }
    }
