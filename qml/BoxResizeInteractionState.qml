...
QtObject {
    ...
    function update(canvasX, canvasY) {
        ...
        const delta = rotatePoint((canvasX - resizeStartCanvasX) / scale,
                                  (canvasY - resizeStartCanvasY) / scale,
                                  -resizeStartRotation)
        ...
        if (activeResizeHandle.indexOf("w") >= 0) left = Math.min(delta.x, right - minimumBoxSize)
        else if (activeResizeHandle.indexOf("e") >= 0) right = Math.max(resizeStartW + delta.x, left + minimumBoxSize)
        if (activeResizeHandle.indexOf("n") >= 0) top = Math.min(delta.y, bottom - minimumBoxSize)
        else if (activeResizeHandle.indexOf("s") >= 0) bottom = Math.max(resizeStartH + delta.y, top + minimumBoxSize)
        resizeW = right - left
        resizeH = bottom - top
    ...
    function bounds() {
        return { x: resizeX, y: resizeY, width: resizeW, height: resizeH }
    }
