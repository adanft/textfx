...
QStringList matchingFamilies(const QString &requested) {
  ...
  for (const QString &family : QFontDatabase::families()) {
    if (family.compare(needle, Qt::CaseInsensitive) == 0 ||
        family.contains(needle, Qt::CaseInsensitive) ||
        needle.contains(family, Qt::CaseInsensitive)) {
...
bool renderSample(const QString &path, const QString &title,
                  const QFont &font) {
  ...
  painter.drawText(QRect(20, 60, 720, 95), Qt::AlignLeft | Qt::AlignVCenter,
                   QStringLiteral("TextFX font sample 0123"));
  painter.end();
...
int main(int argc, char **argv) {
  ...
  if (app.arguments().contains(QStringLiteral("--help")) ||
    ...
    out << "Usage: " << app.applicationName()
        << " [font family] [output-dir]\n";
    out << "Defaults to the first Qt font database family when no family is "
           "provided.\n";
    out << "Set TEXTFX_DIAGNOSTIC_FONT_FILE to load a specific font file "
           "before resolving.\n";
    return 0;
  }
  ...
  const QString outputDir =
      app.arguments().size() > 2 ? app.arguments().at(2) : QString();

  const QString fontFile = QProcessEnvironment::systemEnvironment().value(
      QStringLiteral("TEXTFX_DIAGNOSTIC_FONT_FILE"));
  if (!fontFile.isEmpty()) {
    const int id = QFontDatabase::addApplicationFont(fontFile);
    out << "addApplicationFont: id=" << id << " file=" << quote(fontFile)
        << " families=[";
    const QStringList appFamilies =
        id >= 0 ? QFontDatabase::applicationFontFamilies(id) : QStringList{};
    for (qsizetype i = 0; i < appFamilies.size(); ++i)
      out << (i == 0 ? "" : ", ") << quote(appFamilies.at(i));
    out << "]\n";
  }
  ...
  for (qsizetype i = 0; i < substitutes.size(); ++i)
    out << (i == 0 ? "" : ", ") << quote(substitutes.at(i));
  out << "]\n";
  ...
  for (const QString &family : matches) {
    ...
    for (qsizetype i = 0; i < styles.size(); ++i)
      out << (i == 0 ? "" : ", ") << quote(styles.at(i));
    out << "]\n";
  }

  QFont requestedFont(requested);
  requestedFont.setPixelSize(72);
  const QString styleName = QProcessEnvironment::systemEnvironment().value(
      QStringLiteral("TEXTFX_DIAGNOSTIC_STYLE_NAME"));
  if (!styleName.isEmpty())
    requestedFont.setStyleName(styleName);
  ...
  out << "  qFontInfoFamily: " << quote(QFontInfo(resolved.font).family())
      << "\n";
  out << "  requestedAvailable: "
      << (resolved.requestedAvailable ? "true" : "false") << "\n";
  out << "  exactMatch: " << (resolved.exactMatch ? "true" : "false") << "\n";
  out << "  fellBack: " << (resolved.fellBack ? "true" : "false") << "\n";
  out << "render/export would use fallback: "
      << (resolved.fellBack ? "true" : "false") << "\n";

  printFontInfo(out, QStringLiteral("QFontInfo TextFX resolved"),
                resolved.font);
  ...
  if (!outputDir.isEmpty()) {
    ...
    const QString requestedPath =
        dir.filePath(QStringLiteral("font-requested.png"));
    const QString resolvedPath =
        dir.filePath(QStringLiteral("font-resolved.png"));
    const bool requestedPng = renderSample(
        requestedPath, QStringLiteral("Requested: %1").arg(requested),
        requestedFont);
    const bool resolvedPng = renderSample(
        resolvedPath,
        QStringLiteral("TextFX resolved: %1").arg(resolved.font.family()),
        resolved.font);
    out << "diagnostic PNG requested: " << quote(requestedPath)
        << " saved=" << (requestedPng ? "true" : "false") << "\n";
    out << "diagnostic PNG resolved: " << quote(resolvedPath)
        << " saved=" << (resolvedPng ? "true" : "false") << "\n";
  }
