...
{
    return (static_cast<quint32>(sfntU16(data, offset)) << 16) | sfntU16(data, offset + 2);
}
...
inline QString sfntUtf16be(const QByteArray& bytes)
{
    ...
    for (qsizetype i = 0; i + 1 < bytes.size(); i += 2) text.append(QChar(sfntU16(bytes, i)));
    return text.trimmed();
}

inline QString sfntNameRecord(const QByteArray& bytes, quint16 platformId)
{
    if (platformId == 0 || platformId == 3) return sfntUtf16be(bytes);
...
inline QStringList fontNamesFromSfntData(const QByteArray& data)
{
    ...
    if (nameOffset < 0 || nameOffset + nameLength > data.size() || nameLength < 6) return {};

    const quint16 count = sfntU16(data, nameOffset + 2);
    const qsizetype stringBase = nameOffset + sfntU16(data, nameOffset + 4);
    QSet<QString> names;
    for (quint16 i = 0; i < count; ++i) {
        const qsizetype record = nameOffset + 6 + i * 12;
        if (record + 12 > data.size()) break;
        ...
        if (stringBase + offset + length > data.size()) continue;

        const QString name = sfntNameRecord(data.sliced(stringBase + offset, length), platformId);
        if (!name.isEmpty()) names.insert(name);
    }
    return QStringList(names.begin(), names.end());
}

inline QHash<QString, QString> aliasesForSfntNames(const QStringList& names, const QString& usableFamily)
{
    QHash<QString, QString> aliases;
    if (names.size() < 2 || usableFamily.isEmpty()) return aliases;

    const QSet<QString> nameSet(names.begin(), names.end());
    if (!nameSet.contains(usableFamily)) return aliases;

    for (const QString& name : names) aliases.insert(name.toCaseFolded(), usableFamily);
    return aliases;
}
