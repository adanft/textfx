...
bool pathPointsEqual(const std::vector<Point>& lhs, const std::vector<Point>& rhs)
{
...
void TextBoxEditingService::setRotation(TextBox& box, double degrees) { box.rotationDegrees = degrees; }
...
void TextBoxEditingService::setGradientColorB(TextBox& box, const QString& color) { box.effects.gradientColorB = normalizeHexColor(color, box.effects.gradientColorB); }

void TextBoxEditingService::setPerspectiveEnabled(TextBox& box, bool enabled) { box.effects.perspectiveEnabled = enabled; }
...
void TextBoxEditingService::setPerspectiveHandle(TextBox& box, const QString& corner, double x, double y)
{
...
void TextBoxEditingService::setPathEnabled(TextBox& box, bool enabled)
{
    ...
    if (enabled) ensureDefaultPath(box);
}

void TextBoxEditingService::setPathMode(TextBox& box, int mode) { box.effects.pathMode = std::clamp(mode, 0, 1); }
...
bool TextBoxEditingService::setPathHandle(TextBox& box, int index, double x, double y)
{
    const bool wasPathEnabled = box.effects.pathEnabled;
    const auto previousPoints = box.effects.pathPoints;
    box.effects.pathEnabled = true;
    ensureDefaultPath(box);
    if (index < 0 || index >= static_cast<int>(box.effects.pathPoints.size())) return box.effects.pathEnabled != wasPathEnabled || !pathPointsEqual(box.effects.pathPoints, previousPoints);
