...
namespace textfx {
namespace {
double numberFromJson(const QJsonObject &object, const char *key,
                      double fallback) {
...
std::string stringFromJson(const QJsonObject &object, const char *key,
...
Point pointFromJson(const QJsonValue &value, Point fallback = {}) {
  ...
  return array.size() >= 2
             ? Point{array.at(0).toDouble(), array.at(1).toDouble()}
             : fallback;
}
...
std::vector<Point> pointsFromJson(const QJsonValue &value) {
  ...
  for (const auto &item : value.toArray())
    points.push_back(pointFromJson(item));
  return points;
}
...
TextBox boxFromClipboardJson(const QJsonObject &object) {
  ...
  style.letterSpacing =
      intFromJson(object, "letterSpacing", style.letterSpacing);
  ...
  effects.outlineEnabled =
      boolFromJson(object, "outline", effects.outlineEnabled);
  ...
  effects.shadowColor =
      stringFromJson(object, "shadowColor", effects.shadowColor);
  effects.shadowOffsetX =
      intFromJson(object, "shadowOffsetX", effects.shadowOffsetX);
  effects.shadowOffsetY =
      intFromJson(object, "shadowOffsetY", effects.shadowOffsetY);
  effects.shadowBlurSize =
      intFromJson(object, "shadowBlurSize", effects.shadowBlurSize);
  effects.gradientEnabled =
      boolFromJson(object, "gradient", effects.gradientEnabled);
  effects.gradientDirection =
      intFromJson(object, "gradientDirection", effects.gradientDirection);
  effects.gradientColorA =
      stringFromJson(object, "gradientColorA", effects.gradientColorA);
  effects.gradientColorB =
      stringFromJson(object, "gradientColorB", effects.gradientColorB);
  effects.perspectiveEnabled =
      boolFromJson(object, "perspective", effects.perspectiveEnabled);
  effects.pathEnabled = boolFromJson(object, "path", effects.pathEnabled);
  effects.pathMode =
      std::clamp(intFromJson(object, "pathMode", effects.pathMode), 0, 1);
  effects.perspectiveNw =
      pointFromJson(object.value("perspectiveNw"), effects.perspectiveNw);
  effects.perspectiveNe =
      pointFromJson(object.value("perspectiveNe"), effects.perspectiveNe);
  effects.perspectiveSe =
      pointFromJson(object.value("perspectiveSe"), effects.perspectiveSe);
  effects.perspectiveSw =
      pointFromJson(object.value("perspectiveSw"), effects.perspectiveSw);
...
TextBox
TextBoxClipboardService::deserializeOrPlainText(const QString &clipboardText) {
