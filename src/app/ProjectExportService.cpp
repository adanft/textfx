...
ProjectExportService::ProjectExportService(const ProjectStore& store) : store_(store) {}
...
ExportJobResult ProjectExportService::exportPages(const ExportJob& job) const
{
    ...
    for (const auto& pagePath : job.pagePaths) {
        ...
        if (document == nullptr) {
            pageResult.error = "No document available for export: " + pagePath.string();
            ++result.failed;
            result.pages.push_back(std::move(pageResult));
            continue;
        }

        const auto exportResult = graph.exportPagePngTimed(*document, pagePath, pageResult.exportPath);
        if (exportResult) {
