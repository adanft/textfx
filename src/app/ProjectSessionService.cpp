...
{
    return normalizePageIndex(index, static_cast<std::size_t>(pages.size())) < 0 ? QString{} : pages.at(index);
}
...
int ProjectSessionService::normalizePageIndex(int index, std::size_t pageCount)
{
    return index >= 0 && index < static_cast<int>(pageCount) ? index : -1;
}

std::string ProjectSessionService::pageKey(const std::filesystem::path& pagePath)
{
