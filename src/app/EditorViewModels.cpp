...
QVariantList pointList(const std::vector<Point> &points) {
  ...
  for (const auto &point : points)
    result.push_back(QVariantList{point.x, point.y});
  return result;
}
...
QVariantMap textBoxMap(const TextBox &box, int index) {
  return {
      ...
      {"perspectiveNw",
       QVariantList{box.effects.perspectiveNw.x, box.effects.perspectiveNw.y}},
      {"perspectiveNe",
       QVariantList{box.effects.perspectiveNe.x, box.effects.perspectiveNe.y}},
      {"perspectiveSe",
       QVariantList{box.effects.perspectiveSe.x, box.effects.perspectiveSe.y}},
      {"perspectiveSw",
       QVariantList{box.effects.perspectiveSw.x, box.effects.perspectiveSw.y}},
