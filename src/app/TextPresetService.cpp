...
auto findPresetByName(std::vector<TextPreset>& presets, const std::string& name)
{
    return std::ranges::find_if(presets, [&](const TextPreset& item) { return item.name == name; });
}
...
bool TextPresetService::applySelectedPreset(TextBox& box, const std::vector<TextPreset>& presets, int selectedPresetIndex)
{
    if (!isValidPresetIndex(presets, selectedPresetIndex)) return false;
    box.style = presets.at(static_cast<std::size_t>(selectedPresetIndex)).style;
    return true;
}

bool TextPresetService::addPreset(std::vector<TextPreset>& projectPresets, const TextBox& sourceBox, const QString& name, std::string& preferredName)
{
...
bool TextPresetService::updateSelectedPreset(std::vector<TextPreset>& projectPresets,
...
bool TextPresetService::renameSelectedPreset(std::vector<TextPreset>& projectPresets,
                                             const std::vector<TextPreset>& presets,
                                             int selectedPresetIndex,
                                             const QString& name,
                                             std::string& preferredName)
{
    ...
    const auto oldName = presets.at(static_cast<std::size_t>(selectedPresetIndex)).name;
    ...
    if (std::ranges::any_of(presets, [&](const TextPreset& item) { return item.name == newName; })) return false;

    auto found = findPresetByName(projectPresets, oldName);
    if (found == projectPresets.end()) return false;
...
bool TextPresetService::deleteSelectedPreset(std::vector<TextPreset>& projectPresets, const std::vector<TextPreset>& presets, int selectedPresetIndex)
{
    if (!isValidPresetIndex(presets, selectedPresetIndex)) return false;
    ...
    std::erase_if(projectPresets, [&](const TextPreset& item) { return item.name == name; });
