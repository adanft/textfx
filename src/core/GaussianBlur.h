...
inline QVector<qreal> gaussianKernel(int radius)
{
    ...
    for (qreal& weight : kernel) weight /= sum;
    return kernel;
}
...
inline const QVector<qreal>& cachedGaussianKernel(int radius)
{
    static const auto kernels = [] {
        ...
        for (int i = 0; i <= MaxBlurKernelRadius; ++i) result[static_cast<std::size_t>(i)] = gaussianKernel(i);
        return result;
    }();
    return kernels[static_cast<std::size_t>(std::clamp(radius, 0, MaxBlurKernelRadius))];
}
    ...
    auto pass = [&](const QImage& from, QImage& to, bool vertical) {
        for (int y = 0; y < from.height(); ++y) {
            ...
            for (int x = 0; x < from.width(); ++x) {
                ...
                for (int i = -radius; i <= radius; ++i) {
                    const QRgb px = vertical ? sample(from, x, y + i) : sample(from, x + i, y);
                ...
                out[x] = qRgba(std::clamp<int>(std::lround(r), 0, 255), std::clamp<int>(std::lround(g), 0, 255), std::clamp<int>(std::lround(b), 0, 255), std::clamp<int>(std::lround(a), 0, 255));
            }
        }
    };
