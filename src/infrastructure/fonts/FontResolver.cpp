...
QStringList fontSearchRoots() {
  ...
  for (const QString &path :
    ...
    roots.insert(QDir::cleanPath(path));
#ifdef Q_OS_UNIX
...
bool fontResolvesToKnownName(const QString &family,
                             const QSet<QString> &names) {
  QFont font(family);
  const QString resolved = QFontInfo(font).family().trimmed();
  for (const QString &name : names) {
    if (resolved.compare(name, Qt::CaseInsensitive) == 0)
      return true;
  }
  return false;
}
...
QHash<QString, QString> fontNameAliases() {
  ...
  for (const QString &root : fontSearchRoots()) {
    if (!QDir(root).exists())
      continue;
    QDirIterator files(root, {QStringLiteral("*.ttf"), QStringLiteral("*.otf")},
...
QString bracketAlias(const QString &requested) {
  if (!requested.endsWith(QLatin1Char(']')))
    return {};
...
QString availableFamilyOrAlias(const QString &requested) {
  ...
  if (const QString family = availableFamily(requested);
      !family.isEmpty() && fontResolvesToSelf(family))
    return family;

  static const QHash<QString, QString> aliases = fontNameAliases();
  if (const QString alias = aliases.value(requested.toCaseFolded());
      !alias.isEmpty())
    return alias;

  const QFontInfo info{QFont(requested)};
  if (info.exactMatch()) {
    if (const QString family = availableFamily(info.family());
        !family.isEmpty())
      return family;
    return info.family();
  }
  ...
  for (const QString &substitute : QFont::substitutes(requested)) {
    if (const QString family = availableFamily(substitute.trimmed());
        !family.isEmpty())
      return family;
  }

  if (const QString alias = bracketAlias(requested); !alias.isEmpty()) {
    if (const QString family = availableFamily(alias); !family.isEmpty())
      return family;
...
bool isGenericFamily(const QString &family) {
  return family == QStringLiteral("sans-serif") ||
         family == QStringLiteral("serif") ||
...
ResolvedFont resolveFont(QFont requested) {
  ...
  if (!listedFamily.isEmpty())
    requested.setFamily(listedFamily);

  const QFontInfo requestedInfo(requested);
  result.requestedAvailable = !listedFamily.isEmpty() ||
                              requestedInfo.exactMatch() ||
                              isGenericFamily(result.requestedFamily);
