...
QPainterPath textLayoutPath(const TextLayoutOptions& options, const QFont& font, QStringList* lineTexts, QVector<qreal>* lineXs, QVector<qreal>* lineBaselines)
{
    QPainterPath path;
    ...
    const qreal paintWidth = std::max<qreal>(1.0, options.width - options.inset * 2.0);
    for (const QString& paragraph : paragraphs) {
        ...
        while (true) {
            ...
            if (options.horizontalAlignment == Qt::AlignHCenter) x = options.inset + (paintWidth - line.naturalTextWidth()) / 2.0;
            if (options.horizontalAlignment == Qt::AlignRight) x = options.inset + paintWidth - line.naturalTextWidth();
            lines.append({paragraph.mid(line.textStart(), line.textLength()), x, line.ascent(), line.height()});
        }
        layout.endLayout();
    }
    qreal blockHeight = 0.0;
    for (qsizetype i = 0; i < lines.size(); ++i) blockHeight += lines.at(i).height + (i + 1 < lines.size() ? options.lineSpacing : 0.0);
    qreal y = options.inset + std::max<qreal>(0.0, (options.height - options.inset * 2.0 - blockHeight) / 2.0);
...
QVector<QPointF> layoutPathPoints(const QVector<QPointF>& normalizedPoints, qreal layoutWidth, qreal layoutHeight, bool smooth)
{
    QVector<QPointF> result;
    result.reserve(normalizedPoints.size());
    for (const QPointF& point : normalizedPoints) result.append({layoutWidth * point.x(), layoutHeight * point.y()});
...
bool isNeutralFlatPath(const QVector<QPointF>& normalizedPoints)
{
    ...
    if (std::abs(normalizedPoints.first().x()) > 0.0001 || std::abs(normalizedPoints.last().x() - 1.0) > 0.0001) return false;
    return std::ranges::all_of(normalizedPoints, [](const QPointF& point) { return std::abs(point.y() - 0.5) <= 0.0001; });
}

QPainterPath pathTextLayoutPath(const TextLayoutOptions& options, const QFont& font, const QVector<QPointF>& normalizedPathPoints, bool smoothPath, qreal pathLineSpacing)
{
    QPainterPath path;
    QStringList lines;
    const QVector<QPointF> guidePoints = layoutPathPoints(normalizedPathPoints, options.width, options.height, smoothPath);
    const qreal guideLength = pathLength(guidePoints);
    if (guideLength <= 0.0) return textLayoutPath(options, font);
    ...
    const qreal firstLineOffset = -(lines.size() - 1) * pathLineSpacing * 0.5;
    for (qsizetype lineIndex = 0; lineIndex < lines.size(); ++lineIndex) {
        ...
        const qreal lineOffset = firstLineOffset + lineIndex * pathLineSpacing;
        for (const QChar ch : line) {
            ...
            transform.rotate(std::atan2(sample.tangent.y(), sample.tangent.x()) * 180.0 / kPi);
