...
bool writeFile(const std::filesystem::path& path, const QByteArray& bytes, std::string* error)
{
...
QJsonArray pointToJson(Point point)
{
...
{
    QJsonArray result;
    for (const auto& point : points) result.append(pointToJson(point));
    if (!points.empty()) {
        while (result.size() < 3) result.append(pointToJson(result.size() == 1 ? Point{0.5, 0.5} : Point{1.0, 0.5}));
    }
    return result;
}
...
std::vector<Point> pathPointsFromJson(const QJsonValue& value)
{
    ...
    for (const auto& item : value.toArray()) result.push_back(pointFromJson(item));
    return result;
}
...
{
    TextStyle style;
    style.fontFamily = object.value("font_family").toString(QString::fromStdString(style.fontFamily)).toStdString();
    style.fontSize = object.value("font_size").toInt(style.fontSize);
    style.textColor = object.value("text_color").toString(QString::fromStdString(style.textColor)).toStdString();
    style.lineSpacing = object.value("line_spacing").toInt(style.lineSpacing);
    style.letterSpacing = object.value("letter_spacing").toInt(style.letterSpacing);
    ...
    style.alignment = static_cast<TextAlignment>(object.value("alignment").toInt(static_cast<int>(style.alignment)));
    return style;
}
...
TextBox boxFromJson(const QJsonObject& object)
{
    ...
    effects.outlineEnabled = object.value("outline_enabled").toBool(effects.outlineEnabled);
    effects.outlineColor = object.value("outline_color").toString(QString::fromStdString(effects.outlineColor)).toStdString();
    effects.outlineSize = object.value("outline_size").toInt(effects.outlineSize);
    effects.blurEnabled = object.value("blur_enabled").toBool(effects.blurEnabled);
    effects.blurSize = object.value("blur_size").toInt(effects.blurSize);
    effects.shadowEnabled = object.value("shadow_enabled").toBool(effects.shadowEnabled);
    effects.shadowColor = object.value("shadow_color").toString(QString::fromStdString(effects.shadowColor)).toStdString();
    effects.shadowOffsetX = object.value("shadow_offset_x").toInt(effects.shadowOffsetX);
    effects.shadowOffsetY = object.value("shadow_offset_y").toInt(effects.shadowOffsetY);
    effects.shadowBlurSize = std::clamp(object.value("shadow_blur_size").toInt(effects.shadowBlurSize), 0, MaxShadowBlurSize);
    effects.gradientEnabled = object.value("gradient_enabled").toBool(effects.gradientEnabled);
    effects.gradientDirection = std::clamp(object.value("gradient_direction").toInt(effects.gradientDirection), 0, 1);
    effects.gradientColorA = object.value("gradient_color_a").toString(QString::fromStdString(effects.gradientColorA)).toStdString();
    effects.gradientColorB = object.value("gradient_color_b").toString(QString::fromStdString(effects.gradientColorB)).toStdString();
    effects.pathEnabled = object.value("path_enabled").toBool(effects.pathEnabled);
...
QJsonObject boxToJson(const TextBox& box)
{
    ...
    object.insert("outline_color", QString::fromStdString(box.effects.outlineColor));
    ...
    object.insert("shadow_color", QString::fromStdString(box.effects.shadowColor));
    object.insert("shadow_offset_x", box.effects.shadowOffsetX);
    object.insert("shadow_offset_y", box.effects.shadowOffsetY);
    object.insert("shadow_blur_size", std::clamp(box.effects.shadowBlurSize, 0, MaxShadowBlurSize));
    object.insert("gradient_enabled", box.effects.gradientEnabled);
    object.insert("gradient_direction", std::clamp(box.effects.gradientDirection, 0, 1));
    object.insert("gradient_color_a", QString::fromStdString(box.effects.gradientColorA));
    object.insert("gradient_color_b", QString::fromStdString(box.effects.gradientColorB));
...
bool JsonSerializer::loadPage(const std::filesystem::path& path, DocumentModel& document, std::string* error)
{
    const auto json = QJsonDocument::fromJson(QByteArray::fromStdString(readFile(path)));
    if (!json.isObject()) {
        setError(error, "Save file is not valid JSON.");
        return false;
    }
    const auto root = json.object();
    if (root.value("format").toString() != PageFormat || !root.value("boxes").isArray()) {
        setError(error, "Unsupported page save format.");
        return false;
    }
    document.clear();
    for (const auto& item : root.value("boxes").toArray()) {
        if (item.isObject()) document.textBoxes().push_back(boxFromJson(item.toObject()));
    }
    document.markSaved();
    return true;
}

bool JsonSerializer::savePage(const std::filesystem::path& path, const std::string& pageName, const DocumentModel& document, std::string* error)
{
    QJsonArray boxes;
    for (const auto& box : document.textBoxes()) boxes.append(boxToJson(box));
    const QJsonObject root{{"format", PageFormat}, {"page", QString::fromStdString(pageName)}, {"boxes", boxes}};
    return writeFile(path, QJsonDocument(root).toJson(QJsonDocument::Indented), error);
}

bool JsonSerializer::loadPresets(const std::filesystem::path& path, std::vector<TextPreset>& presets, std::string* error)
{
    const auto json = QJsonDocument::fromJson(QByteArray::fromStdString(readFile(path)));
    if (!json.isObject() || json.object().value("format").toString() != PresetsFormat || !json.object().value("presets").isArray()) {
    ...
    for (const auto& item : json.object().value("presets").toArray()) {
        ...
        if (!name.isEmpty() && !properties.isEmpty()) presets.push_back({name.toStdString(), styleFromJson(properties)});
    }
    return true;
}

bool JsonSerializer::savePresets(const std::filesystem::path& path, const std::vector<TextPreset>& presets, std::string* error)
{
    QJsonArray items;
    for (const auto& preset : presets) {
        if (!preset.name.empty()) items.append(QJsonObject{{"name", QString::fromStdString(preset.name)}, {"properties", styleToJson(preset.style)}});
    }
    return writeFile(path, QJsonDocument(QJsonObject{{"format", PresetsFormat}, {"presets", items}}).toJson(QJsonDocument::Indented), error);
}
